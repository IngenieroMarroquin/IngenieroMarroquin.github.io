...
async function handleActivation() {
    ...
    try {
        ...
        if (response.ok && data.success) {
            // ¡Éxito! Guardamos el estado y mostramos un mensaje final.
            localStorage.setItem(LICENSE_STORAGE_KEY, 'VALID');
            activateBtn.classList.remove('loading');
            activateBtn.textContent = '¡Activado!';
            // Recargamos la página después de un segundo para que el usuario vea el mensaje y la app se inicie correctamente.
            setTimeout(() => {
                window.location.reload();
            }, 1000);
        } else {
            // Fallo, mostramos el mensaje del servidor y restauramos el botón.
...
function checkLicense() {
    ...
    if (licenseStatus === 'VALID') {
        // Licencia válida, ocultamos la pantalla de activación y mostramos la app.
        if (activationStep) {
...
if(backToStep1Btn) backToStep1Btn.addEventListener('click', () => { document.getElementById('step-1-datasheet').style.display = 'block'; navigateToAppStep('step1'); });
...
if(fullResetBtn) fullResetBtn.addEventListener('click', () => { window.location.reload(); });
