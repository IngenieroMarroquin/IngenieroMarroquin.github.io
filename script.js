...
function resetCalibrationState() {
    ...
    calibrationState = {
        instrumentData: {},
        calibrationData: { 
            ideal: [], measured: [], errors: [], 
            ideal_mA: [], measured_mA: [], errors_mA: [] 
        },
        span: 0,
        errorThreshold_mA: 0,
        chart: null,
...
nextBtn1.addEventListener('click', () => {
    if (validateFormStep('1a')) {
        hideError();
        navigateToFormStep('1b');
    }
});
backBtn1.addEventListener('click', () => navigateToFormStep('1a'));

nextBtn2.addEventListener('click', () => {
    if (validateFormStep('1b')) {
        hideError();
        sessionState.executingCompany = sanitizeHTML(document.getElementById('executingCompany').value);
        navigateToFormStep('1c');
    }
});
...
clientTypeSelect.addEventListener('change', () => {
    const isExternal = clientTypeSelect.value === 'Externo';
    clientNameGroup.classList.toggle('hidden', !isExternal);
});

protocolSelect.addEventListener('change', () => {
    const isOther = protocolSelect.value === 'Otro';
    protocolOtherGroup.classList.toggle('hidden', !isOther);
});

pvUnitSelect.addEventListener('change', () => {
    const isCustom = pvUnitSelect.value === 'custom';
...
logoInput.addEventListener('change', (event) => {
    ...
    if (file && (file.type === 'image/jpeg' || file.type === 'image/png')) {
        ...
        showError('Por favor, seleccione un archivo de imagen válido (.jpg o .png).');
    }
});
...
customKeyboard.addEventListener('click', (e) => {
    if (!e.target.matches('.keyboard-btn')) return;
...
instrumentForm.addEventListener('submit', (e) => {
    ...
    if (protocolValue === 'Otro') {
        protocolValue = sanitizeHTML(document.getElementById('protocolOther').value);
    }
    
    let pvUnitValue = sanitizeHTML(document.getElementById('pvUnit').value);
    if (pvUnitValue === 'custom') {
        pvUnitValue = sanitizeHTML(document.getElementById('pvUnitOther').value);
    }
...
resetMeasurementBtn.addEventListener('click', () => {
...
async function generatePDF() {
    ...
    const resultsTableHeight = 35; // Altura estimada
    
    if (pageHeight - finalY - 15 < resultsTableHeight) {
        doc.addPage();
        finalY = 15;
    }
    ...
    offscreenContainer.style.height = '450px';
...
function softReset() {
    ...
    const fieldsToReset = [
        'tag', 'instrumentType', 'brand', 'model', 'serialNumber', 'power', 
        'protocol', 'protocolOther', 'workArea', 'lrv', 'urv', 'pvUnit', 'pvUnitOther', 
        'workOrder', 'testType', 'testCondition', 'clientType', 'clientName', 
        'permissibleError', 'calibrator', 'calibratorSerialNumber', 
        'calibratorLastCal', 'calibratorCalDue', 'technician', 'workLead', 'supervisor'
    ];
