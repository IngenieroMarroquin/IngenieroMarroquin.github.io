...
async function initializeMainApp() {
    ...
    // --- FUNCIONES INTERNAS Y DE LÓGICA ---
    ...
    function validateFormStep(stepId) {
        ...
        for (const el of requiredElements) {
            let value = '';
            if (el.tagName === 'INPUT' || el.tagName === 'SELECT') { value = el.value; } else if (el.tagName === 'DIV') { value = el.textContent; }
    ...
    function prepareCalibrationStep() {
        ...
        idealValuesCells.forEach((cell, index) => { cell.textContent = parseFloat(calibrationState.calibrationData.ideal[index].toPrecision(6)); });
    ...
    async function generatePDF() {
        ...
        const tableBody = calibrationState.calibrationData.ideal.map((ideal, index) => [`${index * 25}%`, ideal.toFixed(2), calibrationState.calibrationData.measured[index].toFixed(2), calibrationState.calibrationData.ideal_mA[index].toFixed(2), calibrationState.calibrationData.measured_mA[index].toFixed(2), calibrationState.calibrationData.errors_mA[index].toFixed(2)]);
        doc.autoTable({ ...tableStyles, startY: finalY, theme: 'grid', margin: { left: 14, right: 14 }, head: [['Punto', `Ideal (${instrumentData.pvUnit})`, `Medido (${instrumentData.pvUnit})`, 'Ideal (mA)', 'Medido (mA)', 'Error (mA)']], body: tableBody, didDrawCell: (data) => { if (data.column.index === 5 && data.cell.section === 'body') { const errorInMA = calibrationState.calibrationData.errors_mA[data.row.index]; const isCellOk = isWithinTolerance(errorInMA, calibrationState.errorThreshold_mA); doc.setFillColor(isCellOk ? 40 : 220, isCellOk ? 167 : 53, isCellOk ? 69 : 69); doc.rect(data.cell.x, data.cell.y, data.cell.width, data.cell.height, 'F'); doc.setTextColor(255, 255, 255); doc.text(data.cell.text[0], data.cell.x + data.cell.padding('left'), data.cell.y + data.cell.height / 2, { baseline: 'middle' }); } } });
        ...
        doc.setFontSize(10); doc.setTextColor(isOk ? 40 : 220, isOk ? 167 : 53, isOk ? 69 : 69); doc.text(`Resultado General: ${isOk ? 'APROBADO' : 'RECHAZADO'}`, 14, finalY); doc.setTextColor(0, 0, 0); finalY += 8;
        ...
        const chartConfig = getChartConfig(); chartConfig.options.animation = false; const tempChart = new Chart(offscreenCanvas.getContext('2d'), chartConfig); await new Promise(resolve => setTimeout(resolve, 200)); const chartImage = tempChart.toBase64Image(); tempChart.destroy(); document.body.removeChild(offscreenContainer);
        ...
        doc.setFontSize(9); doc.setTextColor(80); const margin = 14; const textWidth = doc.internal.pageSize.getWidth() - (margin * 2);
        ...
        const splitText = doc.splitTextToSize(analysisText, textWidth); doc.text(splitText, margin, finalY); doc.setTextColor(0, 0, 0);
    ...
    homeBtn1.addEventListener('click', () => navigateToAppStep('home'));
    ...
    historyListContainer.addEventListener('click', async (e) => {
        if (e.target.classList.contains('btn-delete-report')) {
            const reportId = parseInt(e.target.dataset.id, 10);
            ...
            const filename = e.target.closest('.history-item').dataset.filename;
            alert(`Funcionalidad futura: Abrir el archivo PDF "${filename}" desde la carpeta de descargas del dispositivo.`);
        }
    });
