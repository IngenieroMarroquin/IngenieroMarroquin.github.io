...
async function initializeMainApp() {
    ...
    function scrollToActiveInput(activeElement) {
        ...
        if (elementBottomPosition > keyboardTopPosition) {
            const scrollAmount = elementBottomPosition - keyboardTopPosition + 20;
    ...
    async function renderHistory(filter = {}) {
        ...
        if (filteredReports.length === 0) { historyListContainer.innerHTML = `<p class="empty-history-message">No hay reportes que coincidan con su búsqueda.</p>`; return; }
        filteredReports.forEach(report => {
            const item = document.createElement('div');
            item.className = 'history-item';
            item.dataset.filename = report.pdfFileName;
            item.innerHTML = `<div class="history-item-info"><span class="history-item-tag">${sanitizeHTML(report.tag)}</span><span class="history-item-date">${new Date(report.date).toLocaleString()}</span></div><div class="history-item-actions"><button data-id="${report.id}" class="btn-delete-report" title="Borrar este reporte">🗑️</button></div>`;
    ...
    function softReset() {
        if (calibrationState.chart) { calibrationState.chart.destroy(); }
        ...
        fieldsToReset.forEach(id => { const el = document.getElementById(id); if (el) { if (el.tagName === 'SELECT') { el.selectedIndex = 0; } else if (el.tagName === 'INPUT') { el.value = ''; } else if (el.tagName === 'DIV') { el.textContent = ''; } } });
        document.getElementById('permissibleError').textContent = '1.0';
        document.querySelectorAll('.measured-input').forEach(input => input.textContent = '');
        document.querySelectorAll('#error-values-row td:not(:first-child)').forEach(cell => { cell.textContent = '-'; cell.className = ''; });
        document.getElementById('error-values-row').classList.add('hidden');
        document.querySelectorAll('#ideal-values-row td:not(:first-child)').forEach(cell => cell.textContent = '-');
    ...
    // El resto de las funciones principales de la app que no cambian
    ...
    function calculateEquation() { const { lrv, urv } = calibrationState.instrumentData; calibrationState.span = urv - lrv; const m = 16 / calibrationState.span; const b = 4 - (m * lrv); calibrationState.equation = { m, b, formatted: `Corriente (mA) = ${m.toFixed(4)} * PV ${b >= 0 ? '+' : '-'} ${Math.abs(b).toFixed(4)}` }; }
    function prepareCalibrationStep() { isValidated = false; validateBtn.textContent = 'Validar'; resetMeasurementBtn.disabled = false; backToStep1Btn.disabled = false; const { lrv, urv, pvUnit, permissibleError } = calibrationState.instrumentData; calibrationState.span = urv - lrv; calibrationState.errorThreshold_mA = 16 * (permissibleError / 100); calculateEquation(); const increment = calibrationState.span / 4; calibrationState.calibrationData.ideal = [lrv, lrv + increment, lrv + (2 * increment), lrv + (3 * increment), urv]; document.querySelectorAll('#ideal-values-row td:not(:first-child)').forEach((cell, index) => { cell.textContent = parseFloat(calibrationState.calibrationData.ideal[index].toPrecision(6)); }); document.getElementById('ideal-unit').textContent = pvUnit; document.getElementById('measured-unit').textContent = pvUnit; }
    ...
    async function generatePDF() { const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'letter' }); const { instrumentData, calibrationData, equation } = calibrationState; const isOk = calibrationState.calibrationData.errors_mA.every(e => isWithinTolerance(e, calibrationState.errorThreshold_mA)); let finalY = 10; const pageHeight = doc.internal.pageSize.getHeight(); const footerText = "La validez de este reporte está sujeta a las condiciones del instrumento al momento de la prueba."; const tableStyles = { styles: { fontSize: 9 }, headStyles: { fontSize: 9, fontStyle: 'bold', fillColor: [42, 56, 76] } }; if (sessionState.companyLogo) { try { const imgProps = doc.getImageProperties(sessionState.companyLogo); const logoHeight = 12; const logoWidth = (imgProps.width * logoHeight) / imgProps.height; doc.addImage(sessionState.companyLogo, 'PNG', 14, finalY + 1.5, logoWidth, logoHeight); } catch (e) { console.error("Error al agregar el logo al PDF:", e); } } doc.setFontSize(18); doc.text("Reporte de Calibración de Instrumento", doc.internal.pageSize.getWidth() / 2, finalY + 7.5, { align: "center", baseline: 'middle' }); finalY += 14; if (sessionState.executingCompany) { doc.setFontSize(10); doc.setTextColor(100); doc.text(`Ejecutado por: ${sessionState.executingCompany}`, doc.internal.pageSize.getWidth() / 2, finalY, { align: "center" }); doc.setTextColor(0); finalY += 5; } doc.setFontSize(8); doc.text(`Fecha: ${new Date().toLocaleString('es-ES')}`, doc.internal.pageSize.getWidth() / 2, finalY, { align: "center" }); finalY += 8; doc.setFontSize(11); doc.text("1. Datos del Instrumento", 14, finalY); finalY += 2; doc.autoTable({ ...tableStyles, startY: finalY, theme: 'striped', margin: { left: 14, right: 14 }, head: [['Parámetro', 'Valor']], body: [['TAG', instrumentData.tag], ['Tipo de Instrumento', instrumentData.instrumentType], ['Marca', instrumentData.brand], ['Modelo', instrumentData.model], ['Nº de Serie', instrumentData.serialNumber], ['Alimentación', instrumentData.power], ['Protocolo', instrumentData.protocol], ['Área de Trabajo', instrumentData.workArea], ['Rango', `${instrumentData.lrv} a ${instrumentData.urv} ${instrumentData.pvUnit}`],].filter(row => row[1]), }); finalY = doc.lastAutoTable.finalY + 6; doc.setFontSize(11); doc.text("2. Datos de la Prueba y Equipo Patrón", 14, finalY); finalY += 2; const testConditionMap = { 'As-Found': 'Tal como se encontró (As-Found)', 'As-Left': 'Tal como se dejó (As-Left)' }; const conditionText = testConditionMap[instrumentData.testCondition] || instrumentData.testCondition; let section2Body = [['Orden de Trabajo', instrumentData.workOrder], ['Lugar de la Prueba', instrumentData.testType], ['Condición', conditionText], ['Cliente', instrumentData.clientType], ['Error Permisible', `${instrumentData.permissibleError}% del Span`], ['Equipo Patrón', instrumentData.calibrator], ['Nº de Serie Patrón', instrumentData.calibratorSerialNumber], ['Última Calibración Patrón', instrumentData.calibratorLastCal ? new Date(instrumentData.calibratorLastCal + 'T00:00:00Z').toLocaleDateString('es-ES') : ''], ['Vencimiento Calibración Patrón', instrumentData.calibratorCalDue ? new Date(instrumentData.calibratorCalDue + 'T00:00:00Z').toLocaleDateString('es-ES') : ''],]; if (instrumentData.clientType === 'Externo') { section2Body.splice(4, 0, ['Nombre Cliente', instrumentData.clientName || 'N/A']); } doc.autoTable({ ...tableStyles, startY: finalY, body: section2Body.filter(row => row[1]), theme: 'striped', margin: { left: 14, right: 14 } }); finalY = doc.lastAutoTable.finalY + 6; doc.setFontSize(11); doc.text("3. Resultados de la Prueba de 5 Puntos", 14, finalY); finalY += 2; const tableBody = calibrationState.calibrationData.ideal.map((ideal, index) => [`${index * 25}%`, ideal.toFixed(2), calibrationState.calibrationData.measured[index].toFixed(2), calibrationState.calibrationData.ideal_mA[index].toFixed(2), calibrationState.calibrationData.measured_mA[index].toFixed(2), calibrationState.calibrationData.errors_mA[index].toFixed(2)]); doc.autoTable({ ...tableStyles, startY: finalY, theme: 'grid', margin: { left: 14, right: 14 }, head: [['Punto', `Ideal (${instrumentData.pvUnit})`, `Medido (${instrumentData.pvUnit})`, 'Ideal (mA)', 'Medido (mA)', 'Error (mA)']], body: tableBody, didDrawCell: (data) => { if (data.column.index === 5 && data.cell.section === 'body') { const errorInMA = calibrationState.calibrationData.errors_mA[data.row.index]; const isCellOk = isWithinTolerance(errorInMA, calibrationState.errorThreshold_mA); doc.setFillColor(isCellOk ? 40 : 220, isCellOk ? 167 : 53, isCellOk ? 69 : 69); doc.rect(data.cell.x, data.cell.y, data.cell.width, data.cell.height, 'F'); doc.setTextColor(255, 255, 255); doc.text(data.cell.text[0], data.cell.x + data.cell.padding('left'), data.cell.y + data.cell.height / 2, { baseline: 'middle' }); } } }); doc.setFontSize(8); doc.setTextColor(100); doc.text(footerText, doc.internal.pageSize.getWidth() / 2, pageHeight - 10, { align: "center" }); doc.addPage(); finalY = 15; doc.setFontSize(11); doc.text("4. Conclusión y Gráfico de Linealidad", 14, finalY); finalY += 6; doc.setFontSize(10); doc.setTextColor(isOk ? 40 : 220, isCellOk ? 167 : 53, isOk ? 69 : 69); doc.text(`Resultado General: ${isOk ? 'APROBADO' : 'RECHAZADO'}`, 14, finalY); doc.setTextColor(0, 0, 0); finalY += 8; const offscreenContainer = document.createElement('div'); offscreenContainer.style.position = 'absolute'; offscreenContainer.style.left = '-9999px'; offscreenContainer.style.width = '800px'; offscreenContainer.style.height = '450px'; const offscreenCanvas = document.createElement('canvas'); offscreenContainer.appendChild(offscreenCanvas); document.body.appendChild(offscreenContainer); const chartConfig = getChartConfig(); chartConfig.options.animation = false; const tempChart = new Chart(offscreenCanvas.getContext('2d'), chartConfig); await new Promise(resolve => setTimeout(resolve, 200)); const chartImage = tempChart.toBase64Image(); tempChart.destroy(); document.body.removeChild(offscreenContainer); const pageWidth = doc.internal.pageSize.getWidth(); const chartWidth = pageWidth - 28; const chartHeight = (chartWidth * 450) / 800; doc.addImage(chartImage, 'PNG', 14, finalY, chartWidth, chartHeight); finalY += chartHeight + 8; doc.setFont('courier', 'bold'); doc.setFontSize(9); doc.text(equation.formatted, doc.internal.pageSize.getWidth() / 2, finalY, { align: "center" }); doc.setFont('helvetica', 'normal'); finalY += 8; doc.setFontSize(9); doc.setTextColor(80); const margin = 14; const textWidth = doc.internal.pageSize.getWidth() - (margin * 2); let analysisText = isOk ? `Análisis de Resultados: La desviación máxima registrada durante la prueba se encuentra dentro de la tolerancia de ±${instrumentData.permissibleError}% del span configurado. El instrumento demuestra una respuesta lineal y una exactitud consistentes con las especificaciones del fabricante para su operación nominal.` : `Análisis de Resultados: Se ha identificado una desviación en uno o más puntos de la prueba que excede la tolerancia máxima permisible de ±${instrumentData.permissibleError}% del span. Se recomienda una intervención de ajuste (trimming) y una subsecuente verificación (As-Left) para restablecer la exactitud del instrumento a los parámetros operacionales requeridos.`; const splitText = doc.splitTextToSize(analysisText, textWidth); doc.text(splitText, margin, finalY); doc.setTextColor(0, 0, 0); const signatureY = pageHeight - 50; const signatureBody = [['_________________________\n\n' + (instrumentData.technician || ''), '_________________________\n\n' + (instrumentData.workLead || ''), '_________________________\n\n' + (instrumentData.supervisor || '')], [{ content: 'TÉCNICO EJECUTANTE', styles: { fontStyle: 'bold' } }, { content: 'RESPONSABLE DEL TRABAJO', styles: { fontStyle: 'bold' } }, { content: 'SUPERVISOR DE OPERACIONES', styles: { fontStyle: 'bold' } }]]; doc.autoTable({ ...tableStyles, styles: { ...tableStyles.styles, halign: 'center' }, startY: signatureY, theme: 'plain', margin: { left: 14, right: 14 }, body: signatureBody }); doc.setFontSize(8); doc.setTextColor(100); doc.text(footerText, doc.internal.pageSize.getWidth() / 2, pageHeight - 10, { align: "center" }); doc.save(`Reporte_Calibracion_${instrumentData.tag}.pdf`); try { const reportMetadata = { tag: instrumentData.tag, date: new Date().toISOString(), client: instrumentData.clientName || sessionState.executingCompany || 'Interno', result: isOk ? 'APROBADO' : 'RECHAZADO', pdfFileName: `Reporte_Calibracion_${instrumentData.tag}.pdf` }; await window.dbManager.addReport(reportMetadata); console.log('Metadatos del reporte guardados en el historial offline.'); } catch (error) { console.error('Error al guardar el reporte en el historial offline:', error); } }
    ...
    historyListContainer.addEventListener('click', async (e) => { if (e.target.classList.contains('btn-delete-report')) { const reportId = parseInt(e.target.dataset.id, 10); if (confirm('¿Está seguro de que desea borrar este reporte?')) { await window.dbManager.deleteReport(reportId); await loadAndRenderHistory(); await updateDashboardSummary(); } } else if (e.target.closest('.history-item')) { const filename = e.target.closest('.history-item').dataset.filename; alert(`Funcionalidad futura: Abrir el archivo PDF "${filename}" desde la carpeta de descargas del dispositivo.`); } });
    ...
    if (instrumentForm) instrumentForm.addEventListener('submit', (e) => { e.preventDefault(); prepareCalibrationStep(); navigateToAppStep('step2'); });
...
function registerSW() {
    if ('serviceWorker' in navigator) {
                ...
                newWorker.addEventListener('statechange', () => {
                    if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                        ...
                        if (updateNotification && updateBtn) {
                            ...
                            // Se asigna el evento una sola vez para evitar múltiples recargas
                            updateBtn.onclick = () => {
        ...
        navigator.serviceWorker.addEventListener('controllerchange', () => {
            ...
            // Solo recarga si hay un nuevo service worker esperando para activarse
